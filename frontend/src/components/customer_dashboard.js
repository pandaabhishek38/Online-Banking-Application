...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("change", account_drop_down_closed);
  ...
  const debitCardButton = document.getElementById("debit_card");
  ...
  fetch("/api/accountActivity", {
    ...
    .then((data) => {
      ...
      for (var i = 0; i < accountDetails.length; i++) {
        ...
        map_of_balances[account_info.Account_Number] =
      ...
      if (current_act) {
        document.getElementById("account_balance").textContent =
          map_of_balances[current_act].toFixed(2);
  ...
  function account_drop_down_closed() {
    document.getElementById("account_balance").textContent =
      map_of_balances[document.getElementById("accountDropdown").value].toFixed(
        2
      );
    const current_act = document.getElementById("accountDropdown").value;
    if (current_act) {
    ...
    .then((response) => {
      if (!response.ok) {
        ...
        document.getElementsByClassName(
          "error"
        )[0].textContent = `${response.statusText}, please contact admin`;
      } else {
