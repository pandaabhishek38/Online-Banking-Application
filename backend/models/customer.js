      ...
      getActiveDebitCard: function (customer_id, callback) {
        ...
        db.query(checkAccountQuery, [customer_id], (accountError, accountResults) => {
            ...
            /*const getDebitCardQuery = `
                ...
                AND Card_Status = 0
            `;*/
            const getDebitCardQuery = 'SELECT * FROM Cards WHERE Account_Number = ? AND Card_Type = ? AND Card_Status = ?';
            const values = [accountNumber, 'debit', 0];

            db.query(getDebitCardQuery, values, (cardsError, cardsResults) => {
                if (cardsError) {
                    return callback({ message: 'Error retrieving active debit card details' });
                }
