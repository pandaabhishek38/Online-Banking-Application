...
function handleAPIRequest(req, res) {
  ...
  const endpoint = cleanPath.split("/").pop().trim();
  console.log("Raw pathname:", JSON.stringify(pathname));
  console.log("API Hit:", req.method, pathname, "→ endpoint:", endpoint);
  if (req.method === "POST") {
    switch (endpoint) {
      case "login":
      ...
      case "card_payment":
        ...
        break;
      case "process_debit_card_request":
        authenticateToken(req, res, () =>
          cardController.processDebitCardRequest(req, res)
        );
        break;
      case "admin_process_debit_card_request":
        authenticateToken(req, res, () =>
          adminController.processAdminDebitCardRequest(req, res)
        );
        //adminController.processAdminDebitCardRequest(req, res);
        break;
      case "transfer-funds":
        authenticateToken(req, res, () =>
          fundsTransferController.transfer(req, res)
        );
        break;
      case "int-payment":
    ...
    switch (endpoint) {
      ...
      case "getAllCustomers":
        ...
        break;
      case "get_debit_card_details":
        authenticateToken(req, res, () =>
          customerController.getActiveDebitCard(req, res)
        );
        break;
      case "bankStatement":
        authenticateToken(req, res, () =>
      ...
      case "getAccountNumbers":
        authenticateToken(req, res, () =>
          customerController.getAccountNumbers(req, res)
        );
        break;
      case "getTellers":
      ...
      case "getODAccDetails":
        authenticateToken(req, res, () =>
          customerController.getODAccDetails(req, res)
        );
        break;
      default:
        res.writeHead(404, { "Content-Type": "application/json" });
    ...
    switch (endpoint) {
      ...
      case "toggleOverdraft":
        authenticateToken(req, res, () =>
          customerController.toggleOverdraft(req, res)
        );
        break;
